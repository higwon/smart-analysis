...
| U03 | Product shell rework to UX02 (Stage-first) + operation-launcher / role-switching Inspector framework | U01, U02, F04, **UX02 (doc 26, approved)** | process dialogs | Rewrite (registry-driven) | P1 | ✅ | review (MainWindow recomposed Stage-first per doc 26: recessed Navigator/Inspector rails + elevated Stage; **Analyze ▾** operation launcher; 4-role Inspector (Dataset/Operation/Result/Step); semantic Flatten controls (segmented Scope/Orientation, Order stepper); `image.statistics` Result card (Sq/Sa/skew + histogram) via `IImageAnalysisUseCase`; rail-docked provenance strip (step = read-only, not navigable); Before/After independent-Z compare; backend (use case/operation/viewer) unchanged; directional-hairline tokens added; L/D render verified) |
