...
| F01 | Units + Axes + Buffers (checkpoints F01-A/B/C) | F00 | `FW.Data.Quantity`, `RawToRealTransform`, `ImageBaseScanData` | Reuse unit *semantics* (B); rewrite immutable, no statics | A: UnitRegistry+conversions match legacy · B: Axis raw↔real matches legacy · C: `ScanBuffer<T>` ownership (buffer strategy via ADR) | P0 | ✅ | review |
