...
| FF02 | TIFF writer (WPF-free) + provenance | FF01, F05 | `TiffWriter` (D) | Clean-room | writes result TIFF incl. provenance; round-trip | P1 | ✅ | review (new Application port `IScanFileWriter` + `PsiaTiffWriter` (Infrastructure) — hand-emits the LE TIFF container + PSIA tags (magic/data/header) so `PsiaTiffReader` parses it back; pixels written as float32 gain=1/offset=0 (lossless round-trip); X/Y geometry **canonicalized to µm** (the PSIA header's fixed unit — origin affine, step scale-ratio; non-length axes rejected) so a non-µm axis survives. **Identity + provenance (F05)** embedded as a JSON side-car in the standard `ImageDescription` tag, reusing the P01 workspace DTO records so the provenance JSON shape is identical across both carriers. Reader **extended** to restore id+provenance when the side-car is present, else legacy fresh-id + `Root` (real PSIA files unaffected). No WPF, no TIFF lib for writing (ADR-015). DI `AddPsiaTiffWriter()` wired at the root; 5 tests: full pixels/axes/channel round-trip through the real reader, identity+provenance restore (op id/input/params survive), legacy no-side-car → Root, non-image rejected, DI resolves the port) |
