...
| A07 | Rotate / Flip (geometry) op (split from old Crop/Rotate/Flip/Pixel/Arithmetic umbrella → A07a crop, A07b pixel/arithmetic) | F04 | `RotateFlip` (A) | Clean-room | P1 | ✅ | review (`image.geometry` — clean-room `ImageGeometry` core: horizontal/vertical flip + 90° CW/CCW + 180° rotation, pure row-major reorder like `SpatialFilters`. Quarter turns swap width/height (`SwapsAxes`), so the derived dataset **swaps the X/Y scan axes** (immutable `Axis` records reused); flips/half-turn keep them. Derived image + provenance; schema = single `kind` enum → surfaces under Process, kind→Choice, no shell code; render-verified (flip/rotate of an asymmetric 96×64 raster, 90° → 64×96); 15 tests incl. algebraic identities (quarter∘quarter=half, cw∘ccw=identity, half=flipH∘flipV). Crop (needs D02 ROI) + pixel/arithmetic split out as follow-ups) |
