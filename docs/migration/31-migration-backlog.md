...
| A09 | Grain / particle op | F04 | `GrainDetector`+`SequentialLabeler` (A/B/C) | Clean-room | P2 | ✅ | review (`image.grains` — clean-room `GrainDetector`: 8-connected components (iterative flood fill, no recursion limit) of pixels ≥ a normalized height threshold, dropping regions below a min area. **Measurement** artifact (like A03): GrainCount / Coverage / MeanArea (px) / MeanHeight (Z unit); threshold given as normalized 0–1 mapped into the image min…max (scale-free, reproducible); non-finite pixels excluded + warned. Fourth op on the U08 generic form — surfaces under **Measure**, threshold→Number / minArea→Integer, no shell code; render-verified (5 tall bumps detected, 2 faint rejected); 10 tests incl. 8-connectivity, min-area speck rejection, coverage/mean-height. Physical-area unit (Unit²) a follow-up) |
