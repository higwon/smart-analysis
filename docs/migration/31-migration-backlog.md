...
|---|---|---|---|---|---|---|---|---|
| F00 | Repository & Solution bootstrap (Architecture Gate) | — | none | New (minimal skeleton) | 8 projects (ADR-007) + **dependency-inverted reference skeleton (ADR-009/010): App=composition root, Application/UI ⊄ Infrastructure, `Infrastructure → Application` allowed (Port impl), no cycle** + build/Nullable + test project + **minimal arch guard**; **no commercial libs, no domain/algorithm/parser/UI code** | P0 | ✅ | ready |
