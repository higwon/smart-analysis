...
| UIX05 | Design-system control finishing + theme verification | UIX03 | — | Thin themed **`SA.ScrollBar`** (rounded thumb, no arrows, hover/drag tones) as the implicit default — replaces the grey Windows scrollbar in the Navigator tree / Inspector / provenance list / multi-line text (esp. Dark); reuses existing neutral brushes (no new palette keys). New **`SmartAnalysis.UiTests`** (net8.0-windows) unblocks WPF-dependent tests: `ThemeManager` state/resolve + **live palette-swap** proof (a `SA.Brush.*` resolves to different colours per theme on a real Application) + `ThemePreferenceStore` round-trip/guard | P1 | ✅ | review (ScrollBar L/D render-verified; 10 UiTests; arch-guard updated to 9 projects; 312 tests green) |
