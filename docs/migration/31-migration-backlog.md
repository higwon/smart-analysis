...
| UIX06 | Template the remaining setter-only controls | UIX05 | — | Full `ControlTemplate`s (SA brushes, L/D) for the controls left setter-only after the ComboBox fix — **RadioButton** (accent indicator), **Slider** (rail + accent thumb), **Expander** (rotating chevron header), **Separator**, **TabControl**, **Menu / MenuItem** (all four roles: submenu item/header + top-level item/header, with themed pop-ups) / **ContextMenu**, and **DataGrid** (control template + `DataGridColumnHeader`/`Cell`/`Row` parts + themed scrollbars). No control now falls back to system-coloured WPF chrome; the control-template guard extended to cover them | P2 | ✅ | review (control gallery render-verified in L/D — all 9 controls theme-correct; 15 control-template guard cases; 327 tests green) |
