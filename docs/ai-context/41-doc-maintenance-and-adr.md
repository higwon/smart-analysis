...
| OD-2 | Final XY chart library (ScottPlot vs OxyPlot) | doc 15 / V00 | **decided (ADR-018: ScottPlot 5)** |
...
**buffer ownership strategy (ADR-011: owned array over `Memory<T>`, defer pooling — resolves OD-1)**, **datasets are Id-based entities owning their buffers (ADR-012)**, **provenance record shape v1 (ADR-013: `ProvenanceRecord` = ParentId+Steps; serialization in Infrastructure/P01)**, **operation contract implementation (ADR-014: single-source-of-truth provenance on the output object — no result-level step; `IExecutionEnvironmentProvider` injected into ops; validated `ParameterDescriptor`/`ParameterSchema` + raw-value/Unit-metadata convention; `AddAnalysisOperation`/`AddOperationRegistry` DI surface; MVP defers `Region`/`Quality`/`InPlaceView`)**, **TIFF reader prep (ADR-015: reader standardizes on TiffLibrary/MIT — BitMiracle is legacy write only; reader = Application `IScanFileReader` port + Infrastructure adapter, no FileFormats project; fixtures = committed minimal corpus + env-gated golden dir)**, **summary-statistics empty-input divergence (ADR-016: legacy returns MaxValue/MinValue sentinels on empty input — doc 07 M5; new code returns NaN, verified against the MV00 golden for all non-empty cases)**, **workspace container format (ADR-017: directory-package — manifest.json + little-endian float buffers — behind an Application `IWorkspaceStore` port + Infrastructure adapter; buffers inline, schema v1, typed open-failures; relink/re-run/migration deferred)**, **XY chart library (ADR-018: ScottPlot 5/MIT Candidate→Approved via the V00 spike — 1M-point headless render behind V01 `ICurveView`; UI/viz-impl only, OxyPlot fallback; resolves OD-2)**.
